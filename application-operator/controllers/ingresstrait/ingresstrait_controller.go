...
func (r *Reconciler) createOrUpdateChildResources(ctx context.Context, trait *vzapi.IngressTrait, service *corev1.Service) *reconcileStatus {
...
func (r *Reconciler) updateTraitStatus(ctx context.Context, trait *vzapi.IngressTrait, status *reconcileStatus) (reconcile.Result, error) {
	resources := status.createTypedReferences()
...
func (r *Reconciler) fetchTrait(ctx context.Context, name types.NamespacedName) (*vzapi.IngressTrait, error) {
	var trait vzapi.IngressTrait
	r.Log.Info("Fetch trait", "name", name)
...
func (r *Reconciler) createOrUpdateGateway(ctx context.Context, trait *vzapi.IngressTrait, rule vzapi.IngressRule, name string, status *reconcileStatus) *istioclinet.Gateway {
	...
	ref := cpv1alpha1.TypedReference{APIVersion: gatewayAPIVersion, Kind: gatewayKind, Name: trait.Name}
	status.children = append(status.children, ref)
...
// Results are added to the status object.
func (r *Reconciler) createOrUpdateVirtualService(ctx context.Context, trait *vzapi.IngressTrait, rule vzapi.IngressRule, name string, service *corev1.Service, gateway *istioclinet.Gateway, status *reconcileStatus) {
	...
	ref := cpv1alpha1.TypedReference{APIVersion: virtualServiceAPIVersion, Kind: virtualServiceKind, Name: trait.Name}
	status.children = append(status.children, ref)
	status.results = append(status.results, res)
	status.errors = append(status.errors, err)
...
// The children array is scan looking for Service's APIVersion and Kind.
// If found the unstructed data is converted to a Service object and returned.
