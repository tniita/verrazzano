...
COPY bin/verrazzano-platform-operator /usr/local/bin/verrazzano-platform-operator

# Copy the Verrazzano install and uninstall scripts
WORKDIR /verrazzano
COPY install ./install
COPY uninstall ./uninstall
COPY config/scripts/run.sh .
COPY config/scripts/kubeconfig-template ./config/kubeconfig-template
